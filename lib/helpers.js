...
_beforeEach.withApp = function(app, options) {
  opts = _.extend({ User: 'User', AccessToken: 'AccessToken',
    RoleMapping: 'RoleMapping' }, options);

  if (app.models[opts.User]) {
    // Speed up the password hashing algorithm
    app.models[opts.User].settings.saltWorkFactor = 4;
  }
...
_beforeEach.givenUserWithRole = function (attrs, role, optionalHandler) {
  _beforeEach.givenUser(attrs, function (done) {
    var test = this;
    test.app.models[opts.Role].findOrCreate({name: role}, function (err, result) {
      if(err) {
        console.error(err.message);
        if(err.details) console.error(err.details);
        return done(err);
      ...
      test.app.models[opts.RoleMapping].create(
        {principalId: test.user.id,
         principalType: test.app.models[opts.RoleMapping].USER,
         roleId: result.id},
        function (err, result) {
          if(err) {
            console.error(err.message);
